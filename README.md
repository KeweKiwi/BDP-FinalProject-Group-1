...
At this point, the Python virtual environment should already be active.
...
The dependencies will be installed later in Step 18 before running the Streamlit dashboard, because they are only required for the dashboard layer.

Expected output:

```text
The virtual environment is active and ready for the next steps.
```

### Step 4 - Download and Place the Dataset

Download `2019-Oct.csv` from Kaggle and place it inside:
...
### Step 7 - Check HDFS DataNode Health
...
### Step 9 - Copy Dataset to NameNode Container
...
### Step 10 - Create Raw Folder in HDFS
...
### Step 11 - Upload Dataset to HDFS
...
### Step 12 - Verify Raw Dataset in HDFS
...
### Step 14 - Run Spark SQL Batch Processing
...
### Step 15 - Expected Spark Console Output
...
### Step 16 - Verify Processed Tables in HDFS
...
### Step 17 - Verify Local Dashboard CSV Output
...
If the `output/dashboard` folder does not exist yet, make sure Step 14 has completed successfully because the folder is generated by the Spark SQL batch job.
...
### Step 18 - Install Dependencies and Run Streamlit Dashboard
