...
> The screenshot above displays the Hadoop NameNode Web UI accessed via port 9870. This interface confirms that the HDFS cluster is active and ready to serve as the distributed storage for the raw dataset and the batch-processed results generated by Spark SQL. This status validates that the big data infrastructure is correctly initialized and prepared for data ingestion and processing.
>
...
> This screenshot confirms that the core Docker services are running successfully. Specifically, you should verify that `bdp-namenode`, `bdp-datanode`, and `bdp-spark` are listed with an 'Up' status. These containers form the backbone of the pipeline; if any of these are missing or failing to start, the subsequent data processing and HDFS operations cannot proceed.
>
...
> This screenshot displays the Hadoop NameNode Web UI, accessible via `http://localhost:9870`. This dashboard provides a real-time overview of the cluster's health. You should verify that the NameNode status is 'active' and that at least one live DataNode is registered, confirming that the HDFS cluster is correctly initialized and ready for data storage operations.
>
> ![HDFS Web UI](screenshots/hdfs_web_ui.png)
...
>This screenshot confirms that the `2019-Oct.csv` dataset has been successfully uploaded into the HDFS directory at `/user/bdp/raw/`. The command `hdfs dfs -ls -h` verifies the file's presence and its 5.3GB size. Note that the visible `SASL encryption trust check` logs are standard informational messages from Hadoop and do not indicate an error with the upload process.
> 
...
> This screenshot captures the initial execution of the Spark SQL batch job via `spark-submit`. It demonstrates successful data ingestion by displaying the inferred dataset schema and a summary of event type distributions. This confirms that Spark has correctly loaded the large-scale CSV file from HDFS and is now executing the defined analytical transformations to prepare the data for business intelligence reporting.
>
> ![Spark SQL Output 1](screenshots/spark_sql_output_1.png)

> This screenshot displays the final outputs of the Spark SQL analytical queries, covering key performance indicators (KPIs) such as brand revenue, product category engagement, conversion rates, and the customer behavior funnel. The concluding message, 'Batch SQL Processing Completed', confirms that the job successfully performed the transformations and saved the results both as HDFS tables and as CSV files in the local `output/dashboard` directory, ready for visualization.
>
...
> This screenshot confirms that the Spark SQL job successfully persisted the processed analytical tables into the HDFS directory at `/user/bdp/processed`. Each directory contains the transformed data corresponding to specific business metrics (e.g., brand revenue, conversion rates, and funnel summaries). This step validates that the transformation phase of the pipeline is complete and that the refined datasets are now stored in a distributed, queryable format.
>
...
> "This screenshot confirms that the necessary analytical data has been successfully exported to the `output/dashboard directory`. These CSV files represent the 'ready-to-consume' outputs generated by the Spark SQL batch job. By staging these files locally, the Streamlit dashboard can efficiently load and visualize the KPIs, bridging the gap between distributed big data storage and end-user reporting.
>
...
> This screenshot showcases the Streamlit Dashboard, the frontend layer of this project. It provides an intuitive summary of the processed data, featuring KPI cards that highlight essential business metrics such as total engagement (views/cart/purchases), revenue drivers, and overall conversion rates. The Customer Behavior Funnel visualization effectively illustrates the progression of user intent, allowing stakeholders to identify drop-off points in the customer journey. This dashboard concludes the end-to-end pipeline, translating raw big data into actionable business intelligence.
>
> ![Streamlit Overview and Funnel](screenshots/streamlit_overview_funnel.png)


> "This screenshot details the Revenue Analysis section of the dashboard, offering a deep dive into the top 10 product categories and brands by purchase revenue. By visualizing this data through bar charts alongside detailed tabular metrics, the dashboard provides a clear comparative view of market performance. This enables stakeholders to quickly identify top-performing revenue drivers, such as the dominance of smartphones and the Apple brand from the October 2019 dataset, transforming raw event logs into high-level business intelligence.
>
> ![Streamlit Revenue Analysis](screenshots/streamlit_revenue_analysis.png)


> This final section, the Findings Summary, consolidates the key analytical insights derived from the October 2019 dataset. By explicitly stating the top revenue drivers (e.g., electronics.smartphone and apple) and performance metrics (e.g., a 1.82% overall conversion rate), this summary provides high-level stakeholders with a quick, data-driven synthesis of customer behavior. It effectively demonstrates that the pipeline did more than just move data—it generated meaningful intelligence that highlights the discrepancy between high browsing volume and actual purchasing activity, providing a clear basis for future strategic marketing or UX optimization.
>
